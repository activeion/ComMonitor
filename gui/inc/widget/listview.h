...
template<>
struct wnd_traits<listview> {
	typedef initor::listview initor_t;
};


struct listview : wnd_base<event::base> {
	...
	listview() {
		INIT_P_RW(listview, item_count);
		INIT_P_RW(listview, sel);
		INIT_P_R(listview, sel_count);
		INIT_P_R(listview, col_count);

		bind_property(col,
					  bind(&listview::get_col, this, placeholders::_1),
					  bind(&listview::set_col, this, placeholders::_1, placeholders::_2));
		bind_property(item, 
					  bind(&listview::get_item, this, placeholders::_1, placeholders::_2), 
					  bind(&listview::set_item, this, placeholders::_1, placeholders::_2, placeholders::_3));
	}
...
#define LIST_COLUMN_FLAGS  LVCF_TEXT | LVCF_WIDTH
	...
	listview* insert_col(int idx, const lv_col& col) {
		LVCOLUMN x = {0};
        x.mask = LIST_COLUMN_FLAGS;
		x.pszText = (char*)col.text.c_str();
		x.cx = col.width;
	...
	listview* add_col(const lv_col& col) {
		insert_col(get_col_count(), col);
		return this;
	}
	listview* add_col(const string& text, int width = 80) {
