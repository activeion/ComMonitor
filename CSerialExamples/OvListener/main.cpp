...
int __cdecl _tmain (int /*argc*/, char** /*argv*/)
{
	...
	if (lLastError != ERROR_SUCCESS)
		return ::ShowError(serial.GetLastError(), 
				_T("Unable to set COM-port setting"));
	...
	if (lLastError != ERROR_SUCCESS)
		return ::ShowError(serial.GetLastError(),
				_T("Unable to set COM-port event mask"));
	...
	if (lLastError != ERROR_SUCCESS)
		return ::ShowError(serial.GetLastError(), 
			_T("Unable to set COM-port read timeout."));

	// Create a handle for the overlapped operations
	HANDLE hevtOverlapped = ::CreateEvent(0,TRUE,FALSE,0);;
	if (hevtOverlapped == 0)
		return ::ShowError(serial.GetLastError(),
				_T("Unable to create manual-reset event for overlapped I/O."));

	// Setup the overlapped structure
	OVERLAPPED ov = {0};
	ov.hEvent = hevtOverlapped;

	// Keep reading data, until an CRLF has been received
	bool fContinue = true;
	do
	{
		// Wait for an event
		...
		switch (::WaitForSingleObject(hevtOverlapped,INFINITE))
		{
		case WAIT_OBJECT_0:
			{
				// Save event
